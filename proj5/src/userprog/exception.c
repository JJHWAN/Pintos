...
page_fault (struct intr_frame *f) 
{
  ...
  user = (f->error_code & PF_U) != 0;
   ...
   // if not read only page에 대한 접근 X (not present 참조)
   // 페이지 폴트가 일어난 주소에 대한 spt_entry 구조체 검색
   // spt_entry를 인자로 넘기며 handle_mm_fault() 함수 호출

   // 제대로 파일이 물리 메모리에 로드 되고 맵핑 됐는지 검사

   //printf("page fault %p\t", fault_addr);
