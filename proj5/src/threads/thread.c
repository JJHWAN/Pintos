...
void thread_aging(){
    ...
    // increase priority proportional to the time spent by the ticks increases
  ...
  if(timer_ticks() % TIMER_FREQ == 0){
    // time to update load_avg & recent cpu
    update_load_avg_and_recent_cpu();
  }

  if(timer_ticks() % 4 == 0){
    // time to update priority
    update_priority();
  }
...
bool compare_prior(struct list_elem* a, struct list_elem* b, void* aux){
  struct thread *x, *y;
  x = list_entry(a, struct thread, elem);
  y = list_entry(b, struct thread, elem);
...
void thread_sleep(int64_t ticks){
  ...
  t->wakeup_tick = ticks+start;
  list_push_back(&sleep_list, &(t->elem));
  update_next_tick(ticks+start);
  thread_block();
...
void thread_awake(int64_t ticks){
  ...
  // 인자로 들어온 tick과 비교해서 sleep list에 있는 원소보다 
  // tick이 커지면 ready_list에 삽입한다.

  struct list_elem *ptr, *pt2;
  struct thread* temp = NULL;
  int64_t min;
  int size = list_size(&sleep_list), i;
  ...
  for(i=0;i<size;i++){
    ...
    if(ticks>=temp->wakeup_tick){
      ...
      update_next_tick(temp->wakeup_tick);
      //printf("in there?\n");
      ptr = list_next(ptr);
    }
  }
...
void update_next_tick(int64_t ticks){
  // sleep list에서 가장 작은 wakeup_tick을 가지고 있는 원소를 찾아 
  // update_next_tick을 업데이트한다.
  // 호출하는 경우가 오직 sleep list의 원소의 wakeup_tick
  ...
  for(i=0;i<size;i++){
    temp = list_entry(ptr, struct thread, elem);
    if(temp->wakeup_tick < next_tick_to_awake)
      next_tick_to_awake = temp->wakeup_tick;
  }
               ...
               thread_func *function, void *aux) 
{
  ...
  if(parent->t_dir != NULL)
  { 
	  t->t_dir = dir_reopen(parent->t_dir);
  }
...
int thread_get_load_avg(void){
  //printf("int_mul_float(100,load_avg) / FRACTION : %d\n", int_mul_float(100,load_avg) / FRACTION);
  //printf("and int mul_float(100, (load_avg/FRACTION)) : %d\n", int_mul_float(100,load_avg/FRACTION));
...
init_thread (struct thread *t, const char *name, int priority)
{
  ...
  // thread struc에 요소를 새로 추가한 만큼, 새로운 요소들에 대한 init이 필요하다.
  #ifdef USERPROG
    sema_init(&(t->exit), 0);     
    sema_init(&(t->load), 0);
    sema_init(&(t->mem), 0);
